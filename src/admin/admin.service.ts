...
export class AdminService {
  ...
  async getStats() {
    ...
    const [
      totalMovies, totalUsers, totalReviews,
      moviesByGenre, reviewsThisMonth, reviewsLastMonth,
      usersThisMonth, moviesThisMonth, recentMovies, recentUsers,
      topRatedMovies, avgRatingResult,
    ] = await Promise.all([
      ...
      this.prisma.review.count({ where: { createdAt: { gte: startOfLastMonth, lt: startOfMonth } } }),
    ...
    const topRated = topRatedMovies
      .map((m) => ({
        ...
        avgRating: m.reviews.length > 0
          ? Math.round((m.reviews.reduce((s, r) => s + r.rating, 0) / m.reviews.length) * 10) / 10
          : 0,
      }))
    ...
    return {
      overview: {
        ...
        reviewsGrowth: reviewsLastMonth > 0
      ...
      moviesByGenre: moviesByGenre.map((g) => ({ genre: g.genre, count: g._count.genre })),
      recentMovies, recentUsers, topRatedMovies: topRated,
    };
  }
}
